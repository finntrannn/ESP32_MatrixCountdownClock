...
void TimeManager::begin() {
    ...
    if (rtc.begin(&Wire)) {
        rtcFound = true;
        if (rtc.lostPower()) {
            Serial.println("RTC lost power! Defaulting to compile time before NTP sync.");
            rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
        }

        // 2. Chuyển thời gian khởi động từ RTC vào bộ đếm nội bộ EPS32 (micro-giây)
    ...
    // 3. Đăng ký hàm gọi lại (callback) SNTP để tự động cập nhật lại RTC khi có mạng
...
bool TimeManager::isTimeSynced() const {
    // Nếu Unix time > năm 2000 (~946684800) thì hệ thống đã nhận giờ thành công.
