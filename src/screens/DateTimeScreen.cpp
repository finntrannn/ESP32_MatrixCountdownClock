...
void DateTimeScreen::draw(float dt, DisplayManager& display,
	...
	if (appState.isSensorBgEnabled()) {
		for (int y = 0; y < Panel::kResY; y++) {
			...
			panel->drawFastHLine(
				0, y, Panel::kResX,
				panel->color565(bgBright, bgBright, bgBright + 3));
		}
	}
	...
	if (timeManager.isTimeSynced()) {
		...
		if (appState.isShowYearEnabled()) {
			if (appState.isShortYearEnabled()) {
				snprintf(dateStr, sizeof(dateStr), "%02d%c%02d%c%02d",
						 timeinfo->tm_mday, sep, timeinfo->tm_mon + 1, sep,
						 (timeinfo->tm_year + 1900) % 100);
			} else {
				snprintf(dateStr, sizeof(dateStr), "%02d%c%02d%c%04d",
						 timeinfo->tm_mday, sep, timeinfo->tm_mon + 1, sep,
						 timeinfo->tm_year + 1900);
			}
		} else {
			snprintf(dateStr, sizeof(dateStr), "%02d%c%02d", timeinfo->tm_mday,
					 sep, timeinfo->tm_mon + 1);
		}
