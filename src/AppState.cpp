...
void AppState::load() {
    ...
    showSecondsCountdown_ = preferences_.getBool(NvsKeys::kShowSecsCD, Defaults::kShowSecondsCountdown);
    showSecondsDateTime_  = preferences_.getBool(NvsKeys::kShowSecsDT, Defaults::kShowSecondsDateTime);
    ...
    showMessageEnabled_ = preferences_.getBool(NvsKeys::kShowMessage, Defaults::kShowMessage);
    wifiIconEnabled_ = preferences_.getBool(NvsKeys::kWifiIcon, Defaults::kWifiIcon);
    textPanelContent_ = preferences_.getString(NvsKeys::kTxtContent, Defaults::kTextPanelContent);
    textPanelSpeed_  = preferences_.getInt(NvsKeys::kTxtSpeed, Defaults::kTextPanelSpeed);
    textPanelColor_  = preferences_.getInt(NvsKeys::kTxtColor, Defaults::kTextPanelColor);
    textPanelScroll_ = preferences_.getBool(NvsKeys::kTxtScroll, Defaults::kTextPanelScroll);
    cdDaysColor_     = preferences_.getInt(NvsKeys::kCdDaysClr, Defaults::kCdDaysColor);
    cdTimeColor_     = preferences_.getInt(NvsKeys::kCdTimeClr, Defaults::kCdTimeColor);

    showYear_        = preferences_.getBool(NvsKeys::kShowYear, Defaults::kShowYear);
    shortYear_       = preferences_.getBool(NvsKeys::kShortYear, Defaults::kShortYear);
    dateSeparator_   = preferences_.getInt(NvsKeys::kDateSep, Defaults::kDateSeparator);
    ...
    autoCycle_       = preferences_.getBool(NvsKeys::kAutoCycle, Defaults::kAutoCycle);
    ...
    fwSchedules_     = preferences_.getString(NvsKeys::kFwScheds, Defaults::kFwScheds);
    cycleSequence_   = preferences_.getString(NvsKeys::kCycleSeq, Defaults::kCycleSeq);
...
void AppState::parseFwSchedules() {
    ...
    int idx = 0;
    int startIdx = 0;
    while (startIdx < fwSchedules_.length() && idx < 10) {
        ...
        if (colonIdx > 0 && commaIdx > colonIdx) {
            ...
            fwSchedulesParsed_[idx].minute = slot.substring(colonIdx + 1, commaIdx).toInt();
            fwSchedulesParsed_[idx].duration = slot.substring(commaIdx + 1).toInt();
            idx++;
        }
    }
}
...
void AppState::parseCycleSequence() {
    cycleSequenceLen_ = 0;
    int startIdx = 0;
    while (startIdx < cycleSequence_.length() && cycleSequenceLen_ < 10) {
