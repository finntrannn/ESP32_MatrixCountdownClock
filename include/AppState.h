...
class AppState {
public:
    void lock() const { if (mutex_) xSemaphoreTake(mutex_, portMAX_DELAY); }
    void unlock() const { if (mutex_) xSemaphoreGive(mutex_); }
    /**
    ...
    bool     consumeSplashPlayRequest() {
        if (playSplashReq_) {
            playSplashReq_ = false;
            return true;
        }
        return false;
    }
    void     requestSplashPlay() { playSplashReq_ = true; }
    ...
    bool     isShowSecondsCountdownEnabled() const { return showSecondsCountdown_; }
    bool     isShowSecondsDateTimeEnabled() const { return showSecondsDateTime_; }
    ...
    bool     isShowMessageEnabled() const { return showMessageEnabled_; }
    bool     isWifiIconEnabled()    const { return wifiIconEnabled_; }
    const String& getCustomText()  const { return customText_; }
    const String& getTextPanelContent()  const { return textPanelContent_; }
    int      getTextPanelSpeed()  const { return textPanelSpeed_; }
    int      getTextPanelColor()  const { return textPanelColor_; }
    bool     isTextPanelScrollEnabled() const { return textPanelScroll_; }
    int      getCdDaysColor()   const { return cdDaysColor_; }
    int      getCdTimeColor()   const { return cdTimeColor_; }

    // Date formatting
    bool     isShowYearEnabled()  const { return showYear_; }
    bool     isShortYearEnabled() const { return shortYear_; }
    int      getDateSeparator()   const { return dateSeparator_; }

    // WiFi
    const String& getStaSSID()    const { return staSSID_; }
    const String& getStaPass()    const { return staPass_; }
    const String& getApSSID()     const { return apSSID_; }
    const String& getApPass()     const { return apPass_; }

    // Auto-Cycle
    bool     isAutoCycleEnabled() const { return autoCycle_; }
    int      getTimer0_Countdown() const { return timer0_; }
    int      getTimer1_Sensor()    const { return timer1_; }
    int      getTimer2_DateTime()  const { return timer2_; }
    int      getTimer3_Text()      const { return timer3_; }

    const String& getFwSchedules() const { return fwSchedules_; }
    const String& getCycleSequence() const { return cycleSequence_; }
    ...
    const FwSchedule* getParsedFwSchedules() const { return fwSchedulesParsed_; }
    const int* getParsedCycleSequence(int &len) const { len = cycleSequenceLen_; return cycleSequenceParsed_; }
    ...
    void setScreenModeVolatile(int mode) { screenMode_ = mode; } // Does not save to flash (Anti-wear for auto-cycle)
...
private:
    volatile bool playSplashReq_ = false;
